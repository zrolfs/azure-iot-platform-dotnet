...
export class Maintenance extends Component {
    ...
    getData = (deviceEntities, timeInterval) => {
        const deviceIds = Object.keys(
                deviceEntities || this.props.deviceEntities
            ),
            devices = deviceIds.length ? deviceIds.join(",") : undefined,
            [timeParams] = getIntervalParams(
        ...
        this.subscriptions.push(
            TelemetryService.getActiveAlerts(params)
                ...
                .subscribe(
                        ...
                        const {
                            ...
                            (acc, alertedRule) => {
                                ...
                                return update(acc, {
                                    criticalAlertCount: {
                                        $set:
                                            acc.criticalAlertCount +
                                            (alertedRule.severity ===
                                            Config.ruleSeverity.critical
                                                ? alertedRule.alerts.length
                                                : 0),
                                    },
                                    warningAlertCount: {
                                        $set:
                                            acc.warningAlertCount +
                                            (alertedRule.severity ===
                    ...
                    (alertsError) =>
                        this.setState({ alertsError, alertsIsPending: false })
                )
        );

        this.subscriptions.push(
            IoTHubManagerService.getJobs(jobParams).subscribe(
                (jobs) => {
                    const { failedJobsCount, succeededJobsCount } = jobs.reduce(
                        (acc, { stats = {} }) => ({
                            failedJobsCount:
                                acc.failedJobsCount + (stats.failedCount || 0),
                            succeededJobsCount:
                                acc.succeededJobsCount +
    ...
    setAlertStatus = (alerts, newStatus) => {
        ...
        const updates = alerts.reduce(
            (acc, { id }) =>
                update(acc, {
                    [id]: { $set: { status: { $set: newStatus } } },
                }),
            {}
        );
        this.setState(
    ...
    render() {
        const {
            ...
            alertsWithRulename = alerts.map((alert) => {
                const countPerStatus = alert.alerts.reduce((acc, id) => {
